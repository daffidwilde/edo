...
\begin{tikzpicture}
    ...
    \draw[->] ([xshift=-0.1cm] normal.south) to [out=270, in=90]
        ([yshift=1cm] norm1);
    \draw[->] ([yshift=-0.75cm] norm1.south) -- (-4.5, -5.5);

    \draw[->] ([xshift=0.1cm] normal.south) to [out=270, in=90]
        ([yshift=1cm] norm2) node[right=20pt, yshift=20pt] {\color{blue}
        \scriptsize Sample distribution parameters};
    \draw[->] ([yshift=-0.75cm] norm2.south) -- (4.5, -5.5) node[right=20pt,
        yshift=15pt] {\color{blue} \scriptsize Sample values from distribution};

    \draw[->] (uniform) to [out=270, in=90] ([yshift=1cm] uniform1);
    \draw[->] ([yshift=-0.75cm] uniform1.south) -- (0, -5.5);
